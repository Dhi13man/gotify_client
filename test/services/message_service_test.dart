...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockWebSocketChannel.stream).thenAnswer((_) => wsStreamController.stream);
  ...
  group('getMessages', () {
    test('should return messages when request is successful', () async {
      // Arrange
      final messages = [
        {'id': 1, 'title': 'Test', 'message': 'Hello', 'priority': 5, 'date': '2021-01-01', 'appid': 1},
        {'id': 2, 'title': 'Test 2', 'message': 'World', 'priority': 3, 'date': '2021-01-02', 'appid': 2}
      ];
      
      final mockResponse = MockResponse();
      when(() => mockResponse.statusCode).thenReturn(200);
      when(() => mockResponse.body).thenReturn(jsonEncode({'messages': messages}));
    ...
    test('should handle authentication failure', () async {
      ...
      when(() => mockResponse.body).thenReturn(jsonEncode({'error': 'Unauthorized'}));
      ...
      // Act & Assert
      expect(() => messageService.getMessages(), throwsA(isA<ClientAuthenticationException>()));
    });
    ...
    test('should throw exception when response format is invalid', () async {
      ...
      final mockResponse = MockResponse();
      when(() => mockResponse.statusCode).thenReturn(200);
      when(() => mockResponse.body).thenReturn('{"invalid": "response"}'); // Missing 'messages' key
  ...
  group('sendMessage', () {
    test('should return true when message is sent successfully', () async {
      ...
      verify(() => mockClient.post(
        Uri.parse('$serverUrl$messageEndpoint'),
        headers: {'X-Gotify-Key': 'app_token', 'Content-Type': 'application/json'},
        body: jsonEncode({
          'title': 'Test',
          'message': 'Hello World',
          'priority': 5,
    ...
    test('should throw exception when request fails', () async {
      ...
      when(() => mockResponse.body).thenReturn(jsonEncode({'error': 'Server error'}));
  ...
  group('deleteMessage', () {
    ...
    test('should throw exception when request fails', () async {
      ...
      when(() => mockResponse.body).thenReturn(jsonEncode({'error': 'Message not found'}));
